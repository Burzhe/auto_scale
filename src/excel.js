...
export function readCell(sheet, cellRef) {
  const cell = sheet[cellRef];
  return cell ? cell.v : undefined;
}

export function normalizeCellRef(value) {
  return String(value || '').trim().toUpperCase();
}

export function normalizeAnchorRef(value) {
  return String(value || '').trim();
}

export function readCellValue(sheet, cellRef) {
  if (!cellRef) return undefined;
  return readCell(sheet, normalizeCellRef(cellRef));
}

export function readCellNumber(sheet, cellRef) {
...
export function parseRef(rawRef, defaultSheet) {
...
export function readRefCell(workbook, rawRef, fallbackSheet) {
...
export function findSummaryAnchorsInSheet(sheet, sheetName, labelMap) {
  const anchors = {};
...
export function findCalcSummaryAnchors(workbook) {
  const labels = {
    вескг: 'weightRef',
...
export function parseRefsFromFormula(formulaString) {
...
export function expandRangeRefs(rangeRef, sheetName) {
...
export function expandRanges(refs, defaultSheet) {
...
export function traceCellLeaves(workbook, rootRef, opts = {}) {
  const leaves = [];
  const tree = [];
...
export function findHeaderRow(sheetMatrix, rowIndex) {
...
export function inferDetailRowContext(sheetMatrix, rowIndex) {
  ...
  if (qtyCandidates.length >= 2 && mapping.length != null && mapping.width != null && mapping.area != null) {
    ...
    for (const cand of qtyCandidates) {
      let score = 0;
      let used = 0;
      for (let r = sampleStart; r <= sampleEnd; r += 1) {
...
export function buildDetailBreakdown(workbook, leaves) {
  const details = [];
...
export function computeCoverage(leafSum, totalValue) {
  if (!totalValue) return null;
  return leafSum / totalValue;
}

export function buildDspRates(details) {
  const byThickness = {};
  const totals = {};
  let totalArea = 0;
  let totalCost = 0;
...
export function readAnchorValue(workbook, ref) {
  if (!ref) return null;
...
export function parseSumFormulaRefs(formulaString) {
  if (!formulaString) return [];
...
export function aggregateItemsByName(items) {
  const grouped = new Map();
  items.forEach((item) => {
...
export function buildChipboardBreakdown(workbook, totalRef) {
...
export function getCellFormulaString(cell) {
  if (!cell) return '';
...
export function toFullRef(refLike, defaultSheet) {
  const parsed = parseRef(refLike, defaultSheet);
  if (!parsed) return null;
  return `${parsed.sheetName}!${parsed.cellRef}`;
}

export function decodeRefParts(fullRef, defaultSheet) {
...
export function inferCostTableFallback(workbook, sheetName, anchorValue, materials) {
  const sheet = workbook.Sheets[sheetName];
  if (!sheet) return null;
  ...
  for (let r = 0; r < matrix.length; r += 1) {
    ...
    if (rowText.includes('длин') && rowText.includes('ширин') && rowText.includes('кол')
      && (rowText.includes('цен') || rowText.includes('стоим') || rowText.includes('руб'))) {
      headerRow = r;
      break;
    }
  }
  ...
  const findCol = (pred) => {
    for (let c = 0; c < header.length; c += 1) if (pred(norm(header[c]))) return c;
    return -1;
  };
...
export function buildCostTableBreakdown(workbook, anchorRef, materials) {
...
export function buildCalcSummary(workbook, anchors, materials) {
  const baseValues = {
...
export function parseDimensions(sheet, mapping) {
  let width;
  let depth;
  let height;
...
export function detectBaseCostCell(sheet) {
...
export function parseMaterialDictionary(sheet, mapping) {
  const materials = {};
  const json = XLSX.utils.sheet_to_json(sheet, { header: 1, raw: false });
...
export function parseCorpusDetails(sheet, mapping, materials) {
  const json = XLSX.utils.sheet_to_json(sheet, { header: 1, raw: false });
  const parts = [];
...
export function findFurnitureHeaderRow(json) {
...
export function parseFurniture(sheet, mapping) {
  if (!sheet) return [];
  const json = XLSX.utils.sheet_to_json(sheet, { header: 1, raw: false });
...
export function extractThickness(value) {
  const str = String(value || '');
  const match = str.match(/(\d{2})/);
  return match ? Number(match[1]) : null;
}

export function autoDetectMapping(sheet) {
  const mapping = {};
  const json = XLSX.utils.sheet_to_json(sheet, { header: 1, raw: false });
...
export function autoDetectFurnitureMapping(sheet) {
  const mapping = {};
...
export function resolveAnchors(autoAnchors, overrides, defaultSheet = null) {
...
export function parseExcelWithMapping(workbook, mapping) {
...
export function exportToExcel(spec) {
